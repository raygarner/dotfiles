...
au BufNewFile,BufRead *.tex nnoremap ;n a\documentclass{}<Enter><Enter>\usepackage[backend=biber]{biblatex}<Enter>\addbibresource{/home/ray/Documents/bib/uni.bib}<Enter><Enter>\usepackage{graphicx}<Enter>\usepackage[english]{babel}<Enter>\usepackage{blindtext}<Enter>\usepackage[margin=2cm]{geometry}<Enter>\usepackage{listings}<Enter>\lstset{<Enter><Tab>basicstyle=\normalsize\ttfamily,<Enter>columns=flexible,<Enter>breaklines=true<Enter><Backspace>}<Enter><Enter>\tolerance=1<Enter>\emergencystretch=\maxdimen<Enter>\hyphenpenalty=10000<Enter>\hbadness=10000<Enter><Enter>\title{<++>}<Esc>:pu=strftime('%d/%m/%y')<Enter>i\date{<Esc>$a}<Enter>\author{Ray Garner}<Enter><Enter><++><Esc>:w<Enter>:e<Enter>25k$i
